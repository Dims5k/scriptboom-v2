...
export default async function handler(req, res) {
  ...
  const { text, voice = 'Kore', tone = 'dynamique' } = req.body || {};
  ...
  const style = `voix off française de vidéo TikTok, ton ${String(tone).slice(0, 60)}, rythme vivant et naturel`;
  ...
  const attempts = [
    ...
    ...['gemini-3.8-flash-tts', 'gemini-3.1-flash-tts-preview'].map(model => () => callGoogle(`${BASE}/models/${model}:generateContent`, {
      contents: [{ parts: [{ text: `Lis ce texte en français, ${style} :\n\n${text}` }] }],
